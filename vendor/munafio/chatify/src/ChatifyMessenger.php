...
class ChatifyMessenger
{
    ...
    public function fetchMessage($id){
        ...
        return [
            ...
            'viewType' => ($msg->from_id == session('front_uid')) ? 'sender' : 'default',
            'seen' => $msg->seen,
        ];
    }
    ...
    public function fetchMessagesQuery($user_id){
        return Message::where('from_id',session('front_uid'))->where('to_id',$user_id)
                    ->orWhere('from_id',$user_id)->where('to_id',session('front_uid'));
    }
    ...
    public function countUnseenMessages($user_id){
        return Message::where('from_id',$user_id)->where('to_id',session('front_uid'))->where('seen',0)->count();
    }
    ...
    public function inFavorite($user_id){
        return Favorite::where('user_id', session('front_uid'))
    ...
    public function makeInFavorite($user_id, $action){
        if ($action > 0) {
            ...
            $star->user_id = session('front_uid');
            $star->favorite_id = $user_id;
            $star->save();
            return $star ? true : false;
        }else{
            // UnStar
            $star = Favorite::where('user_id',session('front_uid'))->where('favorite_id',$user_id)->delete();
            return $star ? true : false;
        }
    }
