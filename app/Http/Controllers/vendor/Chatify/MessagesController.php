...
class MessagesController extends Controller
{
    ...
    public function index( $id = null)
    {
        ...
        return view('Chatify::pages.app', [
            ...
            'route' => $route,
            'messengerColor' => session("messenger_color"),
            'dark_mode' => session("dark_mode") < 1 ? 'light' : 'dark',
        ]);
    }
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch = users::where('id', $request['id'])->first();
        }
    ...
    public function getContacts(Request $request)
    {
        ...
        ->where(function ($q) {
            $q->where('ch_messages.from_id', session('front_uid'))
              ->orWhere('ch_messages.to_id', session('front_uid'));
        })
        ...
        $users = $users->where('id','!=',session('front_uid'));
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                if ($user->id != session('front_uid')) {
                    // Get user data
                    $userCollection = users::where('id', $user->id)->first();
                    // return $request['messenger_id'];
    ...
    {
        // Get user data
        $userCollection = users::where('id', $request['user_id'])->first();
    ...
    {
        $favoritesList = null;
        $favorites = Favorite::where('user_id', session('front_uid'));
        foreach ($favorites->get() as $favorite) {
            // get user data
            $user = users::where('id', $favorite->favorite_id)->first();
    ...
    public function search(Request $request)
    {
        ...
        $records = users::where('name', 'LIKE', "%{$input}%");
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? users::where('id', session('front_uid'))->update(['dark_mode' => 1])  // Make Dark
                : users::where('id', session('front_uid'))->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request['messengerColor']) {
            ...
            users::where('id', session('front_uid'))
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (session('avatar') != config('chatify.user_avatar.default')) {
                        $path = storage_path('app/public/' . config('chatify.user_avatar.folder') . '/' . session('avatar'));
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
                    ...
                    $update = users::where('id', session('front_uid'))->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? users::where('id', $request['user_id'])->update(['active_status' => 1])
            : users::where('id', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
